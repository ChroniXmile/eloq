// Script to generate sample matches data for the pool rating system
// This script creates sample match data that can be used as input for the Python rating calculation
...
// Disciplines
const disciplines = ['9-ball', '10-ball'];

// Generate sample matches
function generateSampleMatches(count: number): Match[] {
  const matches: Match[] = [];
  
  // Generate matches for a period of 6 months
  ...
  for (let i = 0; i < count; i++) {
    ...
    matchDate.setDate(matchDate.getDate() + Math.floor(Math.random() * 180));
    ...
    // Random balls per rack (typically 9 for 9-ball, 10 for 10-ball)
    const ballsPerRack = discipline === '9-ball' ? 9 : 10;
    ...
    // For simplicity, we'll assume each rack has approximately balls_per_rack balls pocketed
...
async function main() {
  try {
    ...
    const matches = generateSampleMatches(100);
